...
async function files(directory: string, repository: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) found.push(...await files(path, repository));
    else if (entry.isFile()) found.push(path);
    else throw new Error(`Unsupported public file type: ${relative(repository, path)}`);
  }
  return found;
}

export async function checkPrivateBoundary(repository: string): Promise<void> {
  const failures: string[] = [];
  for (const path of await files(repository, repository)) {
    ...
    let value: string;
    try {
      value = new TextDecoder("utf-8", { fatal: true }).decode(await readFile(path));
    } catch {
      failures.push(`${relativePath} cannot be completely inspected as UTF-8 text.`);
      continue;
    }
...
if (import.meta.main) await checkPrivateBoundary(process.cwd());
